...
int main(void)
{
    uint8_t packets[NR_ELEMENTS][PACKET_SIZE_BYTES];
    uint8_t layer_3_headers[NR_ELEMENTS][HEADERS_SIZE_BYTES];

    clock_t start, end;
    double cpu_time_used;
    uint8_t hash[SHA256_SIZE_BYTES];

    // Populate buffers with random numbers

    srand(time(0));
    printf("Populate random values in the packet of size %d \n", PACKET_SIZE_BYTES);
    for (size_t j = 0; j < NR_ELEMENTS; j++)
    {
        for (size_t i = 0; i < PACKET_SIZE_BYTES; i++)
        {
            packets[j][i] = (char) rand();
            //printf("%x", packets[j][i]);
        }
       // printf("\n");
    }

    printf("Populate random values in the layer_3_headers of size %d \n", HEADERS_SIZE_BYTES);
    for (size_t j = 0; j < NR_ELEMENTS; j++)
    {
        for (size_t i = 0; i < HEADERS_SIZE_BYTES; i++)
        {
            layer_3_headers[j][i] = (char) rand();
            //printf("%x", layer_3_headers[j][i]);
        }
        //printf("\n");
    }
    ...
    for (size_t i = 0; i < NR_ITERATIONS; i++) {
        for (size_t j = 0; j < NR_ELEMENTS; j++)
        {
            sha256(packets[j], PACKET_SIZE_BYTES, hash);
        }
    }
    end = clock();

    cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;
    printf("%d packets of size %d hashing took %f seconds to execute \n", NR_ELEMENTS*NR_ITERATIONS, PACKET_SIZE_BYTES, cpu_time_used);


    // Hash layer_3 headers
    start = clock();
    for (size_t i = 0; i < NR_ITERATIONS; i++) {
        for (size_t j = 0; j < NR_ELEMENTS; j++)
        {
            sha256(layer_3_headers[j], HEADERS_SIZE_BYTES, hash);
        }
    }
    end = clock();
    ...
    printf("%d layer 3 headers of size %d hashing took %f seconds to execute \n", NR_ELEMENTS*NR_ITERATIONS, HEADERS_SIZE_BYTES, cpu_time_used);
